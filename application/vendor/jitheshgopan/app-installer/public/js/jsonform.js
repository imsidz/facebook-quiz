...
// Twitter bootstrap-friendly HTML boilerplate for standard inputs
    ...
    jsonform.elementTypes = {
        ...
        'imageselect': {
            ...
            '<div class="dropdown">' +
            '<a class="btn btn-default<% if (buttonClass && node.value) { %> <%= buttonClass %><% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
            '<% if (node.value) { %><img src="<% if (!node.value.match(/^https?:/)) { %><%= prefix %><% } %><%= node.value %><%= suffix %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
            '</a>' +
            '<div class="dropdown-menu" id="<%= node.id %>_dropdown">' +
            '<div>' +
            '<% _.each(node.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn btn-default<% if (buttonClass) { %> <%= buttonClass %><% } %>" style="max-width:<%= width %>px;max-height:<%= height %>px"><% if (key instanceof Object) { %><img src="<% if (!key.value.match(/^https?:/)) { %><%= prefix %><% } %><%= key.value %><%= suffix %>" alt="<%= key.title %>" /></a><% } else { %><img src="<% if (!key.match(/^https?:/)) { %><%= prefix %><% } %><%= key %><%= suffix %>" alt="" /><% } %></a> <% }); %>' +
            '</div>' +
        ...
        'array': {
            ...
            'onInsert': function (evt, node) {
                ...
                if ($(node.el).sortable) {
                    $('> ul', $nodeid).sortable({cancel: ':input,button,[contenteditable="true"]'});
        ...
        'templateBlocks': {
            'template': '<div id="<%= id %>"><div class="row preview-blocks-container">' +
            '<div class="col-md-12 preview-blocks">' +
            '<%= previewBlocks %>' +
            '</div>' +
            '<div class="col-md-12 editor-blocks">' +
            '<%= children %>' +
            '</div>' +
            '</div>' +
            '<a href="#" class="btn btn-default _jsonform-array-addmore"><i class="glyphicon glyphicon-plus-sign" title="Add new"></i> Add new</a></div>',
            'fieldtemplate': true,
            'array': true,
            'childTemplate': function (inner) {
                return '<div data-idx="<%= node.childPos %>" class="editor-block modal fade">' +
                        '<div class="modal-dialog">' +
                            '<div class="modal-content">' +
                                '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title" id="myModalLabel"><%= node.title ? node.title + " - " + node.childPos : "Edit" %></h4></div>' +
                                '<div class="modal-body">' +
                                inner +
                                '</div>' +
                                '<div class="modal-footer">' +
                                    '<button data-idx="<%= node.childPos %>" type="button" class="btn btn-danger delete-btn" data-dismiss="modal"><span class="fa fa-times"> </span>Delete</button>' +
            ...
            'onBeforeRender': function (data, node) {
                ...
                _.each(node.children, function (child, idx) {
                    var title = child.legend ||
                        child.title ||
                        ('Item ' + (idx + 1));
                    previewBlocks += '<div data-idx="' + idx + '"' +
                        'class ="inline-block preview-block ' + ((idx === 0) ? 'active' : '') + ' "' +
                    ' style="cursor:pointer;">' +
                    title +
                    '</div>';
                });
                data.previewBlocks = previewBlocks;
            },
            'onInsert': function (evt, node) {
                var $nodeid = $(node.el).find('#' + escapeSelector(node.id));
                var boundaries = node.getArrayBoundaries();
                ...
                var moveNodeTo = function (fromIdx, toIdx) {
                    ...
                    var incr = (fromIdx < toIdx) ? 1 : -1;
                    var i = 0;
                    var tabEl = $('.editor-blocks > .editor-block', $nodeid).get(0);
                    for (i = fromIdx; i !== toIdx; i += incr) {
                        node.children[i].switchValuesWith(node.children[i + incr]);
                        node.children[i].render(tabEl);
                        node.children[i + incr].render(tabEl);
                ...
                // Refreshes the list of previewBlocks
                var updatePreviewBlocks = function (selIdx) {
                    var tabs = '';
                    var activateFirstTab = false;
                    if (selIdx === undefined) {
                        selIdx = $('> .preview-blocks-container .preview-block.active', $nodeid).data('idx');
                        if (selIdx) {
                            selIdx = parseInt(selIdx, 10);
                        }
                        else {
                    ...
                    $('> .preview-blocks-container > .preview-blocks', $nodeid).html(previewBlocks);
                    if (activateFirstTab) {
                        $('> .preview-blocks-container .preview-block[data-idx="0"]', $nodeid).addClass('active');
                    }
                    //$('> .preview-blocks-container .preview-block', $nodeid).eq(selIdx).click();
                };

                $nodeid.on('click', '> .preview-blocks-container .editor-block .delete-btn', function (evt) {
                    var idx = $(this).data('idx');
                    deleteItem(idx);
                });

                $nodeid.on('click', '> .preview-blocks-container .preview-block', function (evt) {
                    var closest = $(evt.target).closest('.delete-btn');
                    if($(evt.target).closest('.delete-btn').length)
                        return;
                   var previewBlock = $(evt.currentTarget);
                    var idx = previewBlock.data('idx');
                    $('.editor-block[data-idx="'+ idx +'"]', $nodeid).modal('show');
                });
                ...
                $('> a._jsonform-array-deleteitem', $nodeid).click(function (evt) {
                    var idx = $('> .preview-blocks-container .preview-block.active', $nodeid).data('idx');
                    evt.preventDefault();
                    evt.stopPropagation();
                    deleteItem(idx);
                });
                ...
                if ($(node.el).sortable) {
                    $('> .preview-blocks-container .preview-blocks', $nodeid).sortable({
                        cancel: ':input,button,[contenteditable="true"]',
                        containment: node.el,
                        tolerance: 'pointer'
                    });
                    $('> .preview-blocks-container .preview-blocks', $nodeid).bind('sortstop', function (event, ui) {
                        var idx = $(ui.item).data('idx');
                        var newIdx = $(ui.item).index();
                        moveNodeTo(idx, newIdx);
                        updatePreviewBlocks(newIdx);
                    });
                }
        ...
        'fieldset': {
            'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable panel panel-default<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
            '<% if (id) { %> id="<%= id %>"<% } %>' +
            '>' +
            '<% if (elt.expandable) { %><div class="panel-heading"><% } %>' +
            '<% if (node.title || node.legend) { %><legend><%= node.title || node.legend %></legend><% } %>' +
            '<% if (elt.expandable) { %></div><% } %>' +
            '<% if (elt.expandable) { %><div class="panel-body"><div class="form-group"><% } %>' +
            '<%= children %>' +
            '<% if (elt.expandable) { %></div></div><% } %>' +
            '</fieldset>'
        },
        'advancedfieldset': {
            'template': '<fieldset' +
...
//Allow to access subproperties by splitting "."
            ...
            while (node) {
                if (node.parentNode &&
                    ...
                    if (node.formElement && node.formElement.legend) {
                        ...
                        node.childPos + 1;
                        var valueInLegend = this.formElement.valueInLegend;
                        node.valuesInLegend = node.valuesInLegend || {};
                        /*if ((typeof valueInLegend == "object") && valueInLegend.hasOwnProperty('name')) {
                            formData[valueInLegend.name] = node.valuesInLegend[valueInLegend.name] = isSet(this.value) ? this.value : '';
                        } else {
                            formData.value = isSet(this.value) ? this.value : '';
                        }*/
                        //Assigning the node value legend data
                        if(this.key)
                            node.valuesInLegend[this.key.split('.').pop()] = this.value;
                        _.extend(formData, node.valuesInLegend);
                        try {
    ...
    formNode.prototype.enhance = function () {
        ...
        if (this.formElement) {
            ...
            if (this.legendChild && this.legendChild.formElement) {
                $(this.legendChild.el).bind('keyup change', function (evt) {
                    if (node.formElement && node.formElement.legend && node.parentNode) {
                        ...
                        if(targetElm.attr('id')) {
                            var key = targetElm.attr('id').split('.').pop();
                            node.valuesInLegend[key] = targetElm.val();
                            formData[key] = targetElm.val();
                        }
                        formData.value = targetElm.val();
                        try {
                            node.legend = _.template(node.legend, node.valuesInLegend, valueTemplateSettings);
                        } catch(e){
    ...
    $.fn.jsonForm = function (options) {
        ...
        $('.expandable > .panel-body, .expandable > fieldset', formElt).hide();
        $('.expandable > div > legend', formElt).click(function () {
            var parent = $(this).parents('fieldset');
            parent.toggleClass('expanded');
            $('> .panel-body', parent).slideToggle(100);
        });
